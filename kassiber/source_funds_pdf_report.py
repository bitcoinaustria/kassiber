...
class _SourceFundsPdfBuilder:
    ...
    def cover(self) -> list[Any]:
        ...
        story: list[Any] = [
            self.p("Kassiber Source of Funds Report", "cover_title"),
            self.p("Reviewed local evidence disclosure from a saved immutable case snapshot.", "cover_subtitle"),
            self.kv_table(rows),
        ]
    ...
    def source_mix(self) -> list[Any]:
        ...
        rows = [["Source", "Amount", "Asset", "Count"]]
        ...
        if len(rows) == 1:
            ...
            story.append(
                self.table(rows, widths=(58, 28, 22, 20), compact=True, right_columns={1, 3})
            )
        return story
