...
def _ensure_custody_component_replication_schema(conn):
    """Remove constraints that reject valid concurrent/reordered revisions.

    Custody component revisions are authored replicated records.  Two peers can
    independently create the same next revision or activate competing
    revisions.  Both rows must survive replay so application validation can
    make them ineffective and expose the conflict for review.  Likewise,
    ``supersedes`` and ``superseded_by`` form a two-way link whose headers can
    arrive in either order, so they cannot be immediate self foreign keys.

    The first component implementation shipped these as database uniqueness
    constraints/self-FKs.  SQLite cannot drop them in place, hence this small
    idempotent table rebuild.  Child rows are preserved with foreign-key
    enforcement disabled only for the transactional schema swap.
    """

    table = conn.execute(
        "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'custody_components'"
    ).fetchone()
    if not table:
        return
    foreign_keys = conn.execute("PRAGMA foreign_key_list(custody_components)").fetchall()
    has_self_reference = any(
        row["table"] == "custody_components"
        and row["from"] in {"supersedes_component_id", "superseded_by_component_id"}
        for row in foreign_keys
    )
    unique_lineage_index = False
    for index in conn.execute("PRAGMA index_list(custody_components)").fetchall():
        ...
        if columns in {
            ...
            ("profile_id", "lineage_id", "revision"),
        }:
            unique_lineage_index = True
            break
    if not has_self_reference and not unique_lineage_index:
        return

    previous_fk_state = int(conn.execute("PRAGMA foreign_keys").fetchone()[0])
    previous_legacy_alter = int(conn.execute("PRAGMA legacy_alter_table").fetchone()[0])
    conn.commit()
    ...
    try:
        conn.executescript(
            """
            BEGIN IMMEDIATE;
            DROP TRIGGER IF EXISTS trg_custody_component_memberships_supersede;
            DROP INDEX IF EXISTS idx_custody_components_lineage_active;
            DROP INDEX IF EXISTS idx_custody_components_lineage_draft;
            DROP INDEX IF EXISTS idx_custody_components_lineage_revision;
            DROP INDEX IF EXISTS idx_custody_components_profile_state;
            DROP INDEX IF EXISTS idx_custody_components_supersedes;
            ALTER TABLE custody_components RENAME TO custody_components__replication_legacy;
            CREATE TABLE custody_components (
                id TEXT PRIMARY KEY,
                lineage_id TEXT NOT NULL,
                workspace_id TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
                profile_id TEXT NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
            ...
            SELECT
                ...
                superseded_by_component_id, activated_at, superseded_at,
                created_at
            FROM custody_components__replication_legacy;
            DROP TABLE custody_components__replication_legacy;

            CREATE INDEX idx_custody_components_lineage_active
                ON custody_components(profile_id, lineage_id) WHERE state = 'active';
            CREATE INDEX idx_custody_components_lineage_draft
                ON custody_components(profile_id, lineage_id) WHERE state = 'draft';
            CREATE INDEX idx_custody_components_lineage_revision
                ON custody_components(profile_id, lineage_id, revision, id);
            CREATE INDEX idx_custody_components_profile_state
                ON custody_components(profile_id, state, created_at DESC, id DESC);
            CREATE INDEX idx_custody_components_supersedes
                ON custody_components(supersedes_component_id)
                WHERE supersedes_component_id IS NOT NULL;

            CREATE TRIGGER trg_custody_component_memberships_supersede
            AFTER UPDATE OF state ON custody_components
            WHEN OLD.state = 'active' AND NEW.state != 'active'
            BEGIN
                DELETE FROM custody_component_transaction_memberships
        ...
        conn.execute(
            f"PRAGMA legacy_alter_table = {'ON' if previous_legacy_alter else 'OFF'}"
        )
