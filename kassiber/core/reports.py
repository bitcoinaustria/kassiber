...
from .privacy_linkage import analyze_psbt_privacy
...
def report_privacy_mirror(
    ...
    *,
    ...
    """Project shared local chain evidence for the active profile and audience."""
    from .privacy_mirror import build_privacy_mirror

    workspace, profile = hooks.resolve_scope(conn, workspace_ref, profile_ref)
    payload = build_privacy_mirror(conn, profile["id"], redacted=redacted)
    payload["scope"] = {
        "workspace": "active", "profile": "active",
        "workspace_selected": bool(workspace["id"]),
        "profile_selected": bool(profile["id"]),
    }
    return payload
...
def privacy_mirror_table_rows(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    rows = [{
        "surface": "finding", "severity": finding["severity"],
        "item": finding["code"], "evidence_level": finding["authority"],
        "detail": finding["detail"],
    } for finding in payload["findings"]]
    rows.extend({
        "surface": "coverage", "severity": "info", "item": check["code"],
        "evidence_level": check["status"],
        "detail": f"{check['evaluated']}/{check['eligible']} evaluated" + (
            f"; {check['reason']}" if check.get("reason") else ""
        ),
    } for check in payload["coverage"]["checks"])
    return rows
    ...
    *,
    ...
    try:
        ...
        raise AppError(
            ...
            code="validation",
            hint="Provide a valid base64 PSBT v0 or v2 with the required transaction fields.",
