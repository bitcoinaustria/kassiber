...
def reset_current_profile_data(
    ...
    *,
    ...
    removed = {
        ...
        "chain_analysis_datasets": _count_profile_rows(conn, "chain_analysis_datasets", profile_id),
        "chain_analysis_dataset_claims": _count_sql(conn, "SELECT COUNT(*) FROM chain_analysis_dataset_claims WHERE dataset_id IN (SELECT id FROM chain_analysis_datasets WHERE profile_id=?)", (profile_id,)),
        ...
        "journal_custody_reconciliations": _count_profile_rows(
            conn, "journal_custody_reconciliations", profile_id
        ),
        "custody_authored_evidence_snapshots": _count_profile_rows(
            conn, "custody_authored_evidence_snapshots", profile_id
        ),
        "review_workflow_receipts": _count_profile_rows(conn, "review_workflow_receipts", profile_id),
