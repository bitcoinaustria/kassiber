...
- Supporting modules (bottom-up — no back-edges into the CLI layer):
  ...
  - [kassiber/core/engines/](kassiber/core/engines/) — shared tax-engine input/result types plus the single `GenericRP2TaxEngine` implementation. Both the generic RP2 path and the Austrian (§ 27b EStG) path use it; AT profiles surface rp2's `rp2.plugin.country.at.AT` plugin directly so accounting methods and engine semantics come from rp2, while Kassiber keeps Austrian disposal bucketing / Kennzahl mapping on its side.
  ...
  - [kassiber/core/chain_observer/](kassiber/core/chain_observer/) — observation-only BDK 3.0.0 / LWK 0.18.0 boundary for supported Bitcoin/Liquid Electrum descriptor wallets. Esplora uses the named compatibility transport (`http_route_policy` for otherwise supported configurations) because the pinned SDK bindings cannot enforce redirect/proxy policy; direct SDK Esplora construction is blocked too. Dependency state is versioned inside the main SQLCipher database; fetch completes before the atomic wallet savepoint and no builder/signer/broadcaster crosses the contract.
  - [kassiber/core/sync_backends.py](kassiber/core/sync_backends.py) — capability routing plus explicitly named Esplora/Electrum protocol fetchers and the first-class `bitcoinrpc` live-sync observer. Bitcoin address scripts, Bitcoin Core RPC, and Silent Payments are named by responsibility rather than treated as failed BDK fallbacks. Bitcoin Core descriptor discovery is read-only/local; ranged `importdescriptors` + Core's blocking rescan live inside `bitcoinrpc_sync_adapter`, with per-branch range ends and observed `highest_used` persisted in the freshness checkpoint. BDK/LWK own supported Electrum descriptor discovery; `scan_compatibility_descriptor_targets` handles Esplora and configurations the native bindings cannot represent, including Tor/custom trust/auth/timeout cases. Sync results expose the route and reason; restoring SDK Esplora requires executable redirect/proxy-policy controls in the packaged binding.
  ...
  - [kassiber/core/chain_analysis/](kassiber/core/chain_analysis/) — immutable local observation index, bounded trace/path queries and pure clustering/pattern/partition analytics. The CLI, desktop and AI share [chain_analysis_api.py](kassiber/core/chain_analysis_api.py); acquisition, saved cases/labels and provider projection stay in separate focused modules. Apply observer visibility before filters and analytics. Physical connectivity, private ownership, hypotheses and current journal custody relations have distinct authority. Acquisition is explicit, genesis-bound and bounded; no public fallback or redirects. Remote models receive opaque process/book-scoped graph references; desktop assistant handoffs must use `ui.chain_analysis.ai_context`, never interpolate raw graph identities. Acquisition AI artifacts require an on-device provider and once-only consent. Privacy Mirror (`core/privacy_mirror.py`) projects the same `analyze_snapshot` result into personal relevance and coverage; it has no separate detector, graph or score. Desktop handoffs keep physical identities local; CLI/AI use the shared opaque-reference projection. Shared `features.py` and `psbt.py` own structural facts and PSBT v0/v2/Payjoin validation; `entropy.py` counts explicit conditional models, never ownership probabilities. Dataset packs use exact full subject keys and versioned provenance; failed staging is invisible. Native source grants and cancellable jobs live in `chain_analysis_runtime.py`, with worker-owned database connections in `daemon_chain_analysis.py`. AI dataset imports require a completed source/recipe/hash-bound preview and on-device consent; never rescan large sources on the daemon consent thread. See [local-chain-analysis.md](docs/reference/local-chain-analysis.md).
  - [kassiber/core/chain_analysis_backfill.py](kassiber/core/chain_analysis_backfill.py) — recurring source authorization and resumable Bitcoin Core block observation. Book/domain/source revisions and lifetime quota are rechecked before requests and publication. Source assertions preserve block occurrences and reversible membership; they never grant ownership or custody. Worker execution uses the watch lifecycle; no execution permission or derived state replicates. See [recurring-chain-acquisition.md](docs/reference/recurring-chain-acquisition.md).
  ...
  - [kassiber/core/review_workflow.py](kassiber/core/review_workflow.py) — shared agent/CLI review interface: canonical paginated cases, portable version-bound mixed-operation plans computed on an isolated memory snapshot, atomic apply/rebuild and durable idempotent receipts. `review {cases,plan,apply,receipt}` and `ui.review.{cases,plan,apply,receipt}` cross this same seam. Read-only `ui.review.request_input` produces case/version-bound connection, history-import or attachment handoffs; the UI resumes the same conversation after successful input and daemon `expected_scope` guards bind dialog actions to the original book. The AI apply tool requires once-only consent against server-recomputed effects. Receipt verification describes the historical rebuild, not present report readiness. It delegates bookkeeping to `custody_journal`; do not add an agent-specific accounting interpreter.
  - [kassiber/core/source_funds_review.py](kassiber/core/source_funds_review.py) — target-scoped provenance inspection and missing-input handoffs shared by `source-funds {review-context,request-input}`, AI and desktop. Canonical report recipes and fingerprints bind source/link/evidence edits independently of journal freshness. Handoffs reuse the chat input cards and durable attachment picker; source-funds provenance never grants custody or tax authority. Coverage shares immutable report inputs only within one read snapshot. Saved ZIP evidence must match the frozen attachment hashes before atomic output replacement. See [docs/reference/source-of-funds-review.md](docs/reference/source-of-funds-review.md).
  ...
  - [kassiber/core/filed_report_chain.py](kassiber/core/filed_report_chain.py) — local report-input observation dependencies attached to saved snapshots and the existing change inbox. Sync publishes scoped evidence changes atomically; report-ready rebuilds append exact impact comparisons. Missing coverage stays uncertain, historical exports stay immutable, and acknowledgement never asserts an amended filing. This local observation history does not replicate.
...
- Fresh installs use the OS-native per-user app-data root (`$XDG_DATA_HOME/kassiber` on Linux, `~/Library/Application Support/at.bitcoinaustria.kassiber` on macOS, and `%LOCALAPPDATA%\\at.bitcoinaustria.kassiber` on Windows). On first default launch, `~/.kassiber` state moves there with an atomic same-filesystem rename when the target is absent and the layout is safe; any top-level entry except `bin` or `run` counts as state. Unsupported or conflicting layouts remain on the legacy root, `~/.kassiber/{bin,run}` stay fixed, and `--data-root` / `--env-file` still override selection.
- The CLI model is:
  ...
  - transactions
  - attachments
  - documents (including evidence-only prior tax reports)
...
- In-app AI read tools are explicit daemon kinds, not generic CLI or daemon
  ...
  `ui.source_funds.{evidence.list,coverage,cases.list,review_context,request_input}`,
  `ui.reports.exit_tax_preview`, `ui.egress.snapshot`,
  `ui.custody.lineage.snapshot`,
  `ui.wallets.utxos`, `ui.wallets.identify`, `ui.wallets.analyze_file`,
  `ui.maintenance.settings`, `ui.workspace.health`,
...
- External network invariant: launch, route mount, read-only display,
  ...
  the BDK/LWK observers, shared backend HTTP/SOCKS and Python Electrum
  transports, and chain-analysis acquisition honor destination-blind — it
...
- `wallets {kinds,list,create,get,update,delete,reveal-descriptor,sync,sync-btcpay,attach-btcpay,attach-bullbitcoin-wallet,derive,identify,import-json,import-csv,import-btcpay,import-phoenix,import-river,import-bull,import-coinfinity,import-21bitcoin,import-strike,import-ledger-live,import-cointracking,import-blockpit,import-ledger,ledger-template,analyze-file,preview-document,import-document,import-samourai}`
...
- `documents {list,create,attach,import-report}`
...
- `imports {list,rollback}` — file-import runs; `rollback` reports the plan and needs `--confirm` to delete
...
- Both `generic` and `at` profiles use `GenericRP2TaxEngine` from `kassiber/core/engines/rp2.py`; Austrian profiles select `rp2.plugin.country.at.AT`.
...
- The RP2 engine owns the per-profile journal orchestration: transfer detection, manual-pair application, per-asset grouping, normalized event preparation, and holdings aggregation all live in `kassiber/core/engines/rp2.py`, while CLI handlers only load rows and persist the resulting journal state.
...
- Fully owned native N:M events reuse `ownership_transfers.derive_multi_source_consolidations` and `allocate_msat_fifo`; their allocation cells are accounting coordinates, not physical per-satoshi provenance. `core/transfer_chronology.py` captures current observer-backed input dependencies before custody slicing and orders same-block MOVEs without inventing wallet cycles. `core/custody_native_transitions.py` feeds solo exact native HTLC claims/refunds into the same custody arbitrator without authoring reviewed components; full-population evidence/conflicts precede dismissal/occupancy checks and profile tax policy. LWK and Core RPC HTLC attestations are non-secret facts bound to matching closed observer provenance, never trusted import markers.
...
- Profiles support `generic` and `at` (Austrian, § 27b EStG) tax policies. AT profiles delegate engine defaults to `rp2.plugin.country.at.AT` (`moving_average_at`, accepted accounting methods, `open_positions`, English fallback), while Kassiber consumes rp2's `classify_disposal()` API to persist Austrian semantic buckets and current Kennzahl mappings. Typed Austrian fields on `NormalizedTaxEvent` (`at_regime`, `at_swap_link`) plus the country-neutral event `cost_basis_pool_id` and transfer `from_cost_basis_pool_id` / `to_cost_basis_pool_id` are Kassiber's internal source of truth for the marker wire format; rp2 owns native carried-basis computation. See [docs/austrian-handoff.md](docs/austrian-handoff.md) for the full current carry-basis contract.
...
- Preserve the existing BTC subledger and personal workflows. The explicitly
  ...
  #546 owns ordinary opaque task execution and exact local consent. This
  dependent `codex/accounting-selected-ai` cut (#550) adds selected financial
  disclosure/proposals with their provider protections and tests. Keep
...
- Prefer the external Kassiber CLI Agent Skill from `bitcoinaustria/kassiber-skill`
  ...
  bundle.
- Before calling work push-ready, run `./scripts/quality-gate.sh`. It checks
  Python, TypeScript, ESLint and Vitest.
...
uv run --locked python -m kassiber wallets import-cointracking --help
uv run --locked python -m kassiber wallets import-blockpit --help
uv run --locked python -m kassiber wallets import-ledger --help
uv run --locked python -m kassiber wallets ledger-template --help
uv run --locked python -m kassiber documents import-report --help
...
- Phoenix Lightning wallet CSV import is implemented (`wallets import-phoenix`). River Bitcoin Activity / Account Activity CSV import is implemented (`wallets import-river` and `--source-format river_csv`). Bull Bitcoin order CSV import is implemented as exchange evidence (`wallets import-bull` and `--source-format bullbitcoin_csv`), while Bull's unified wallet transaction CSV is wallet-scoped (`--source-format bullbitcoin_wallet_csv`) and can be split by `bullbitcoin_wallet_network` or mapped onto existing wallets with `wallets attach-bullbitcoin-wallet`. Coinfinity order CSV import is implemented (`wallets import-coinfinity` and `--source-format coinfinity_csv`). 21bitcoin transaction CSV import is implemented (`wallets import-21bitcoin` and `--source-format 21bitcoin_csv`). Strike CSV import is implemented (`wallets import-strike` and `--source-format strike_csv`). CoinTracking and Blockpit migrations use their dedicated transaction CSV schemas (`wallets import-cointracking`, `wallets import-blockpit`): only BTC/LBTC rows are imported, source rows stay in `raw_json`, and tax-ambiguous labels fail closed. Existing tax reports are evidence-only external documents through `documents import-report` / `ui.documents.import_report`; provider totals and lot assignments are not imported as ledger facts. A generic manual ledger importer is implemented (`wallets import-ledger`, source format `generic_ledger`): a fill-in `.xlsx` (read via `openpyxl`) or CSV/TSV template whose `Type` column maps onto real `(direction, kind)` pairs, one Bitcoin leg per row, with the fiat leg becoming exact `exchange_execution` pricing. `wallets ledger-template` writes the blank template (no DB; `.xlsx` via XlsxWriter or `.csv` by extension), and `ui.transactions.ledger_template` is its desktop kind; the desktop import reuses `ui.wallets.import_file` with `source_format=generic_ledger`. A preview-before-import path is also available: `wallets import-ledger --dry-run` (no DB) and the read-only `ui.wallets.ledger_preview` daemon kind both reuse the loader + per-row normalizer to return `{rows_read, mapped, errors, problems[], preview[]}` without persisting (collecting every rejected row's problem at once, unlike the real import which stops at the first); the desktop Generic-ledger setup panel renders that preview before the import action. Local-AI photo/PDF OCR drafts are implemented through `wallets preview-document`, `wallets import-document`, and desktop `ui.wallets.document_import.{preview,import}`; this path is hard-local (loopback Ollama vision/OCR models only), returns ready/quarantined draft rows with confidence metadata, and attaches the source document to imported/enriched transactions. Non-template files are auto-detected: `infer_ledger_columns(header)` remaps an arbitrary export's columns (date; a Type/Side column, or direction/sign, or separate sent/received columns; fee; fiat currency/price/value; note; tx id) onto the ledger shape and feeds the same normalizer (taxonomy + exact pricing preserved); rows without an explicit type become `Buy`/`Sell` when a cash counterleg is present, otherwise `Deposit`/`Withdrawal` by direction. Unrecognized columns raise `ledger_unrecognized` (preview returns `confident: false` + detected columns). Template files keep the native path. An explicit `column_map` overrides the guess.
- Unsupported-exchange onboarding (no predefined importer) is implemented in [kassiber/core/file_analysis.py](kassiber/core/file_analysis.py) plus `normalize_column_map` / `LEDGER_PLAN_FIELDS` in [kassiber/importers.py](kassiber/importers.py). `wallets analyze-file` (no DB, no network, no AI call) reports file kind, headers, the inferred plan, and a dry-run; `--column-map JSON|@path` on `analyze-file` and `import-ledger` supplies a plan when auto-detection fails, threaded through `load_import_records` and daemon `ui.wallets.analyze_file` / `column_map` on `ui.wallets.ledger_preview` + `ui.wallets.import_file`. `column_map` gained `type_map` (translate a file's own Type values onto canonical ledger Types) and `*_header_asset` hints; German column names (`Typ`/`Art`/…) and row values (`Kauf`/`Verkauf`/`Einzahlung`/`Auszahlung`/…) are auto-detected, while direction-less `Überweisung`/`Transfer` deliberately is not. An unrecognized Type *column* is the dangerous failure, not a loud one: with no type and no direction column the direction comes from the amount's sign, so an all-positive export books sales as deposits with nothing rejected — `analyze_file` reports `row_kinds_from_amount_sign` / `next_step.action = map_row_kinds` rather than refusing, since a transfers-only export legitimately has neither column. Two sibling signals for the same class of silent default: `amount_units_unconfirmed` (no column and no header states the asset, and the magnitudes are implausible for BTC — a satoshi column would import 1e8 high) and `header_is_preamble` (the first row is narrower than the rows below it, so it is an account-holder/date-range line the importer misread as column names; it is also withheld from off-device models, since content heuristics cannot tell "Account holder,Max Mustermann" from two column names). Validation lives inside `_ledger_source_records`, so **no entry point can consume an unvalidated plan** — it rejects unknown fields, columns absent from the file, one column claimed by two fields, and `type_map` targets that are not real Types. The invariant: **a plan maps names, never values**; every amount/date/asset still comes from the file through the same normalizer, so a wrong column mapping yields a rejected row rather than a wrong number. Two carve-outs to know: `*_header_asset` IS a value (a wrong hint imports 1e8 off with no rejected row), so it is rejected when it contradicts a header that already states an asset and is refused outright from chat; and `type_map` is guaranteed to name a Type that *exists*, not the *right* one (on an amount-only file the direction is derived from the mapped Type, so nothing contradicts an inversion) — it needs human review before import.
- AI-assisted file analysis: `ai.chat` accepts `attachment: {token, label}` (an opaque staging grant from `internal.document_import.stage`, never a path — the renderer and the model both only hold a token), surfaced as `kassiber chat --file/--file-context` and the desktop composer's attach button (Tauri `pick_chat_attachment_source`, bridge `purpose: "chat_attachment"`, both native-picker-only like the OCR staging command). The read-only AI tool `ui.wallets.analyze_file` takes **no file argument** and always reads that attachment. Egress policy: for an off-device provider the analysis is reduced by `redact_for_egress` to headers, counts, and the column plan — no cell values, no filename, no local path, and per-row problem messages collapsed to a count, since a header row is all it takes to propose a mapping. Crucially a "header" is only disclosed when the file has one: `analyze_file` takes the first non-empty row, which for a preamble line or headerless export is data, so `redact_headers_for_egress` judges the row as a whole and withholds all of it (text cells included — a counterparty is indistinguishable from a column name) if any cell looks like an amount/date/hash/IBAN/email. `inferred_plan`, `applied_plan` and `detected` echo header strings and are redacted the same way. Only a strictly on-device provider (`_provider_is_on_device`: `kind = local` **and** a loopback host, reusing `document_import._is_loopback_host`) may see sample rows. PDFs/photos are not analyzed here; they route to the hard-local OCR path. Downstream cleanup (quarantine, pricing, gaps) deliberately reuses existing tools rather than one orchestrated mega-tool. **The chat loop ends at the plan**: there is no import tool in the AI catalog (and `ui.wallets.import_file` takes a path, not a staging token), no wallet-create tool, and no `ui.imports.*` tools, so the assistant hands the user a `column_map` to run with `wallets import-ledger` and rollback stays a GUI/CLI action. The tool description says so explicitly, because a model with a finished plan and no import tool will otherwise claim it imported.
- Per-import rollback lives in [kassiber/core/import_batches.py](kassiber/core/import_batches.py) with tables `import_batches` + `import_batch_transactions`. Every file import records a run (source format, filename, confirmed `column_map`, counts); `imports rollback` / `ui.imports.rollback` deletes just that run's transactions. **Only rows a run created are linked** — `insert_wallet_records`' `inserted_records` vs `updated_records` — so enrichment of pre-existing rows is never undone, and an insert-nothing import records no run. Rows attached to deleted transactions cascade away (tags, attachments, metadata history, pairs, custody components), which is why `plan_rollback` is a separate pure step the UI shows before `confirm="DELETE"`. Journals are invalidated after a rollback. The tables are deliberately outside replication: the transactions replicate, the record of which local file produced them does not.
- Connection asset badges follow **observed** assets, not configured intent: `_connections` exposes `observedAssets` (from `import_batches.observed_assets`, ordered by row count) plus `importBatchCount` / `lastImportAt`, and `connectionAssetLabel` prefers them. A file import declares no chain, so a `custom` wallet holding L-BTC rows would otherwise show a BTC icon. `sourceFormat` also falls back to the latest run's format, so a wallet created without `--source-format` still describes itself.
- Note on file sizes (per the working rule above): this feature added ~+370 lines to [kassiber/daemon.py](kassiber/daemon.py) and ~+240 to [kassiber/importers.py](kassiber/importers.py) without splitting either. Deliberate — the daemon additions are kind handlers that must live beside the dispatch table, and the importer additions (`normalize_column_map`, `_ledger_type_map`) belong beside `infer_ledger_columns`, which produces the shape they validate; moving them would invert the `core` → `importers` layering. The genuinely new behavior went into two new modules instead ([core/file_analysis.py](kassiber/core/file_analysis.py), [core/import_batches.py](kassiber/core/import_batches.py)).
