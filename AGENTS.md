...
- Unsupported-exchange onboarding (no predefined importer) is implemented in [kassiber/core/file_analysis.py](kassiber/core/file_analysis.py) plus `normalize_column_map` / `LEDGER_PLAN_FIELDS` in [kassiber/importers.py](kassiber/importers.py). `wallets analyze-file` (no DB, no network, no AI call) reports file kind, headers, the inferred plan, and a dry-run; `--column-map JSON|@path` on `analyze-file` and `import-ledger` supplies a plan when auto-detection fails, threaded through `load_import_records` and daemon `ui.wallets.analyze_file` / `column_map` on `ui.wallets.ledger_preview` + `ui.wallets.import_file`. `column_map` gained `type_map` (translate a file's own Type values onto canonical ledger Types) and `*_header_asset` hints; German row values (`Kauf`/`Verkauf`/`Einzahlung`/`Auszahlung`/…) are auto-detected, while direction-less `Überweisung`/`Transfer` deliberately is not. Validation lives inside `_ledger_source_records`, so **no entry point can consume an unvalidated plan** — it rejects unknown fields, columns absent from the file, one column claimed by two fields, and `type_map` targets that are not real Types. The invariant: **a plan maps names, never values**; every amount/date/asset still comes from the file through the same normalizer, so a wrong column mapping yields a rejected row rather than a wrong number. Two carve-outs to know: `*_header_asset` IS a value (a wrong hint imports 1e8 off with no rejected row), so it is rejected when it contradicts a header that already states an asset and is refused outright from chat; and `type_map` is guaranteed to name a Type that *exists*, not the *right* one (on an amount-only file the direction is derived from the mapped Type, so nothing contradicts an inversion) — it needs human review before import.
...
- Wallet config carries an optional `custodial` flag (`CUSTODIAL_CONFIG_KEY`, in `WALLET_SAFE_CONFIG_FIELDS` + the replication allowlist). It is the user's declaration, not inferred: an exchange ledger and a self-custody export can be the same CSV shape, and owned scripts only prove watching. `null` means "not stated" rather than defaulting either way.
