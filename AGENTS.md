...
- `transfers {pair,list,unpair,payouts {list,create,delete},suggest,bulk-pair,dismiss,rules {list,create,apply,delete,enable,disable},components {list,show,create,update,activate,supersede,undo,bulk-resolve}}` — `bulk-pair`, `rules apply`, and `components bulk-resolve` accept `--dry-run`; custody-component preview is pure, apply requires its `--expected-fingerprint`, and explicit `untracked_wallet` placeholder creation occurs only during apply.
