...
- `wallets {kinds,list,create,get,update,delete,reveal-descriptor,sync,sync-btcpay,attach-btcpay,attach-bullbitcoin-wallet,derive,identify,import-json,import-csv,import-btcpay,import-phoenix,import-river,import-bull,import-coinfinity,import-21bitcoin,import-strike,import-samourai,import-mapped-csv,csv-example,mapping-template}`
...
- Generic CSV import for arbitrary wallet/exchange exports is implemented: the pure engine `kassiber/core/csv_mapping.py` **auto-detects** columns (`infer_mapping`) from common header aliases and builds a declarative mapping spec, with a fill-in **example** template (`example_csv`). CLI: `wallets csv-example`, `wallets import-mapped-csv [--mapping] [--dry-run]` (mapping optional → auto-detect; `csv_mapping_unrecognized` when columns can't be recognized), and `wallets mapping-template` for advanced manual specs. Daemon kinds `ui.wallets.csv_example` / `csv_inspect` / `csv_preview` (mapping optional) / `import_mapped_csv`. Desktop: the **CSV import** connection in the Add Connection modal (download example → choose file → auto-detect preview → confirm; Advanced manual-mapping fallback) — `ui-tauri/src/components/kb/csv-mapping/CsvImportPanel.tsx`. See [docs/reference/csv-mapping.md](docs/reference/csv-mapping.md). Per-import only: no persisted/named mappings yet.
