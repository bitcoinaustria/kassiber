...
- Supporting modules (bottom-up — no back-edges into the CLI layer):
  ...
  - [kassiber/core/sync_backends.py](kassiber/core/sync_backends.py) — capability routing plus explicitly named compatibility Esplora/Electrum fetchers and the separate `bitcoinrpc` live-sync adapter. Bitcoin Core descriptor discovery is read-only/local; ranged `importdescriptors` + Core's blocking rescan live inside `bitcoinrpc_sync_adapter`, with per-branch range ends and observed `highest_used` persisted in the freshness checkpoint. The old manual Esplora/Electrum descriptor gap scanner is deleted; BDK/LWK own supported-route discovery.
  ...
  - [kassiber/core/custody_journal.py](kassiber/core/custody_journal.py) — projects complete custody components into chronological synthetic journal legs while retaining real transaction FK anchors. Supports 1:N, N:1, N:M, same-wallet refunds, missing-wallet hops through `untracked` wallets, BTC/LBTC/Lightning rails, component-wide atomic quarantine, and downstream-only tax policy.
