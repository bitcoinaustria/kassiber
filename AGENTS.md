...
- Supporting modules (bottom-up — no back-edges into the CLI layer):
  ...
  - [kassiber/core/custody_gaps.py](kassiber/core/custody_gaps.py) — deterministic advisory discovery plus the shared local projection. Each replaceable projection is keyed by journal input version, exact evidence and both review/accounting ignored-boundary hashes; current UI/AI reads reuse the journal builder's population, while its accounting subset is matched with interpreter-blocked rows excluded before competition. A bounded retention window protects the latest builder projection from provisional review churn. Typed candidate rows and normalized boundary relations feed accounting; privacy-safe presentation rows are separately versioned by review/readiness state and use `(projection_id, sort_group, ordinal, gap_id)` keyset pagination. The legacy serialized snapshot table is never read or written and is cleared after normalized replacement.
