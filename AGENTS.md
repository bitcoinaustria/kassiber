...
- `transfers {pair,list,unpair,payouts {list,create,delete},suggest,bulk-pair,dismiss,rules {list,create,apply,delete,enable,disable},components {list,show,create,update,activate,supersede,undo,bulk-resolve}}` — `bulk-pair`, `rules apply`, and `components bulk-resolve` accept `--dry-run`; custody bulk resolution supports explicit `untracked_wallet` placeholder creation.
...
- Manual pairing via `transfers pair / list / unpair` (table `transaction_pairs`) overrides auto-detection: `apply_manual_pairs` in `kassiber/transfers.py` filters out any auto-pair that touches a manually-paired row. Pair rows persist `swap_fee_msat`, `swap_fee_kind`, `confidence_at_pair`, and `pair_source` (one of `manual` / `bulk_exact` / `bulk_selected` / `rule_auto`); `unpair` soft-deletes via `deleted_at` instead of hard-DELETE so the audit row survives and the partial unique indexes immediately stop blocking re-pair of the same legs. Same-asset carrying-value pairs feed the IntraTransaction path. Reviewed BTC ↔ LBTC rail changes may carry value on every profile while `bitcoin_rail_carrying_value` is enabled; Austrian policy additionally supports reviewed carrying-value treatment for other eligible crypto conversions. Explicit taxable policy keeps SELL + BUY treatment. Detection remains country-neutral. Same-wallet cross-asset pairs are allowed so peg-ins/peg-outs do not require duplicate wallet records. Direct swap payouts (`transfers payouts create`) model a source-wallet outbound that a swap provider settles directly to an external recipient; reviewed proceeds and any tax-policy-specific conversion handling remain downstream of the evidence graph.
...
`uv sync --frozen`. Use `uv run --frozen` for Python commands so the checked-in
lock remains the only development dependency graph. A plain `pip install` is a
packaging compatibility contract, not an alternate contributor workflow. For
...
uv run --frozen python -m kassiber wallets sync-btcpay --help
uv run --frozen python -m kassiber wallets identify --help
uv run --frozen python -m kassiber wallets import-river --help
uv run --frozen python -m kassiber wallets import-coinfinity --help
uv run --frozen python -m kassiber wallets import-21bitcoin --help
uv run --frozen python -m kassiber wallets import-strike --help
uv run --frozen python -m kassiber wallets import-ledger --help
uv run --frozen python -m kassiber wallets ledger-template --help
...
uv run --frozen python -m kassiber journals transfers list --help
uv run --frozen python -m kassiber reports summary --help
uv run --frozen python -m kassiber reports tax-summary --help
uv run --frozen python -m kassiber reports austrian-e1kv --help
uv run --frozen python -m kassiber reports austrian-tax-summary --help
uv run --frozen python -m kassiber reports export-austrian --help
uv run --frozen python -m kassiber reports export-austrian-e1kv-xlsx --help
uv run --frozen python -m kassiber reports export-austrian-e1kv-csv --help
uv run --frozen python -m kassiber reports balance-history --help
uv run --frozen python -m kassiber rates --help
uv run --frozen python -m kassiber diagnostics collect --help
uv run --frozen python -m kassiber chat --help
uv run --frozen python -m kassiber chats --help
uv run --frozen python -m kassiber ai --help
uv run --frozen python -m kassiber ai providers --help
uv run --frozen python -m kassiber ai providers create --help
```
