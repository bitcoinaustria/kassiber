...
class OutputInventoryTest(unittest.TestCase):
    ...
    def test_inventory_rows_preserve_sanitized_privacy_evidence(self):
        sync_state = WalletSyncState(
            chain="bitcoin",
            network="mainnet",
            descriptor_plan=None,
        ...
        active_txid = "ee" * 32
        spent_txid = "ff" * 32
        with tempfile.TemporaryDirectory(prefix="kassiber-utxo-privacy-") as tmp:
            conn = open_db(Path(tmp) / "data")
            try:
                profile, wallet = _seed_wallet(conn)
                    ...
                    [
                        {
                            ...
                            "confirmation_status": "confirmed",
                            "anonymity_score": 42,
                            "excluded_from_coinjoin": False,
                            "key_state": "clean",
                            "anon_history": [{"score": 21}, {"score": 42}],
                        },
                        {
                            "txid": spent_txid,
                            "vout": 1,
                            ...
                            "chain": "bitcoin",
                            "network": "mainnet",
                            "spent_by": active_txid,
                            "excluded_from_coinjoin": True,
                        },
                    ],
                    seen_at="2026-01-01T12:00:00Z",
                )
                ...
                self.assertEqual([row["outpoint"] for row in active], [f"{active_txid}:0"])
                self.assertEqual(active[0]["anonymity_score"], 42)
                self.assertFalse(active[0]["excluded_from_coinjoin"])
                self.assertEqual(active[0]["key_state"], "clean")
                self.assertEqual(active[0]["anon_history"], [{"score": 21}, {"score": 42}])
                ...
                spent = [row for row in all_rows if row["outpoint"] == f"{spent_txid}:1"][0]
                self.assertEqual(spent["spent_by"], active_txid)
                self.assertTrue(spent["excluded_from_coinjoin"])
                self.assertEqual(spent["source"]["spent_at"], "2026-01-01T12:00:00Z")
            finally:
                conn.close()
