...
class CustodySchemaTests(unittest.TestCase):
    ...
    def test_open_db_migrates_immediate_revision_constraints_without_losing_links(self):
        with tempfile.TemporaryDirectory() as root:
            conn = open_db(root)
            _scope(conn)
            conn.commit()
            ...
            try:
                legacy.executescript(
                    ...
                    DROP TABLE custody_component_allocations;
                    DROP TABLE custody_component_transaction_memberships;
                    DROP TABLE custody_component_legs;
                    DROP TABLE custody_components;
                    CREATE TABLE custody_components (
                        ...
                        notes TEXT,
                        change_reason TEXT,
                        supersedes_component_id TEXT REFERENCES custody_components(id),
                        superseded_by_component_id TEXT REFERENCES custody_components(id),
                        activated_at TEXT,
                        superseded_at TEXT,
                        created_at TEXT NOT NULL,
                        UNIQUE (profile_id, lineage_id, revision)
                    );
                    CREATE UNIQUE INDEX idx_custody_components_lineage_active
                        ON custody_components(profile_id, lineage_id) WHERE state = 'active';
                    CREATE UNIQUE INDEX idx_custody_components_lineage_draft
                        ON custody_components(profile_id, lineage_id) WHERE state = 'draft';
                    INSERT INTO custody_components(
                        id, lineage_id, workspace_id, profile_id, revision,
                        component_type, state, superseded_by_component_id,
                        superseded_at, created_at
                    ) VALUES(
                        'old', 'lineage', 'ws', 'profile', 1,
                        'native_transfer', 'superseded', 'new',
                        '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z'
                    );
                    INSERT INTO custody_components(
                        id, lineage_id, workspace_id, profile_id, revision,
                        component_type, state, supersedes_component_id,
                        created_at
                    ) VALUES(
                        'new', 'lineage', 'ws', 'profile', 2,
                        'native_transfer', 'draft', 'old',
            ...
            try:
                old = migrated.execute(
                    "SELECT * FROM custody_components WHERE id = 'old'"
                ).fetchone()
                new = migrated.execute(
                    "SELECT * FROM custody_components WHERE id = 'new'"
                ).fetchone()
                self.assertEqual("new", old["superseded_by_component_id"])
                self.assertEqual("old", new["supersedes_component_id"])
                self.assertEqual(0, old["expected_leg_count"])
                self.assertEqual(0, old["expected_allocation_count"])
                self.assertEqual(0, new["expected_leg_count"])
                self.assertEqual(0, new["expected_allocation_count"])
                self.assertFalse(
                    any(
                        row["table"] == "custody_components"
                        for row in migrated.execute(
                            "PRAGMA foreign_key_list(custody_components)"
                        )
                    )
                )
                indexes = {
                    row["name"]: bool(row["unique"])
                    for row in migrated.execute("PRAGMA index_list(custody_components)")
                }
                self.assertFalse(indexes["idx_custody_components_lineage_active"])
                self.assertFalse(indexes["idx_custody_components_lineage_draft"])
                migrated.execute(
                    """
                    INSERT INTO custody_components(
                        id, lineage_id, workspace_id, profile_id, revision,
                        component_type, state, created_at
                    ) VALUES('concurrent', 'lineage', 'ws', 'profile', 2,
                             'native_transfer', 'draft', ?)
                    """,
                    (NOW,),
                )
