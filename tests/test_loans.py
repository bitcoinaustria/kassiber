...
class LoanTaxClassificationTest(unittest.TestCase):
    ...
    def test_principal_received_and_repaid_are_suppressed(self):
        rows = [
            _row("principal-in", "inbound", ONE_BTC, "2025-05-01T00:00:00Z"),
            _row("principal-out", "outbound", ONE_BTC, "2025-06-01T00:00:00Z"),
        ]
        baseline = _run(rows)
        self.assertTrue(_has_disposal(baseline))
        ...
        result = _run(
            rows,
            [
                {"transaction_id": "principal-in", "role": "loan_principal_received"},
                {"transaction_id": "principal-out", "role": "loan_principal_repaid"},
            ],
        )
