...
const mock = vi.hoisted(() => ({ invoke: vi.fn(), reads: vi.fn(), bound: true }));
vi.mock("@/daemon/client", async original => ({
  ...await original<typeof import("@/daemon/client")>(),
  useDaemon: (kind: string, args?: unknown, options?: unknown) => { mock.reads(kind, args, options); return ({ data: { data: kind === "ui.backends.options" ? { backends: [
...
beforeEach(() => { mock.invoke.mockClear(); mock.reads.mockClear(); mock.bound = true; });
...
it("does not poll sources while the disclosure is closed", () => {
  renderToStaticMarkup(<SourceAcquisitionPanel onError={() => {}} />);
  expect(mock.reads).toHaveBeenCalledWith("ui.chain_analysis.sources.list", { limit: 100 }, { enabled: false, staleTime: 0, refetchInterval: false });
});
