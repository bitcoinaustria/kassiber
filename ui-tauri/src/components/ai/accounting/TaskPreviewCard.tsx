...
import { isTaskEntry, isTaskPreview, isTaskReports, type TaskPreview, type TaskProposal } from "./taskModel";
...
export function TaskPreviewCard({ value, book }: { value: TaskPreview; book: MoneyBook }) {
  ...
  return <section className="space-y-3 rounded-lg border bg-muted/20 p-4" aria-label={t("preview")}>
    ...
    {value.proposals?.map((proposal) => <EntryPreview key={`${proposal.source_kind}:${proposal.source_id}`} entry={proposal.payload} book={book} source={proposal.source_id} />)}
...
function EntryPreview({ entry, book, source }: { entry: TaskProposal["payload"]; book: MoneyBook; source?: string }) {
  const { t } = useTranslation("accountingReview");
