...
export function graphSelectionPosition(
  layout: EvidenceGraphLayout,
  edges: Pick<EvidenceGraphEdge, "id" | "source" | "target">[],
...
export function initialGraphCamera(
  layout: EvidenceGraphLayout,
  viewport: GraphViewport,
  nodes: Pick<EvidenceGraphNode, "id" | "preferInitialFocus">[],
  edges: Pick<EvidenceGraphEdge, "id" | "source" | "target">[],
  selection: ViewportSelection | null,
  initialView: "fit" | "readable" = "readable",
): GraphCamera {
  if (initialView === "fit") return fitGraphCamera(layout, viewport);
  ...
  // Prefer caller-designated anchor candidates, then the leftmost visible node.
  // This is a visual anchor only; it does not assert an ownership or flow root.
  const candidates = nodes.some((node) => node.preferInitialFocus)
    ? nodes.filter((node) => node.preferInitialFocus)
    : nodes;
