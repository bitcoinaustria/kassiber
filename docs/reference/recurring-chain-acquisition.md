...
Verification lives in `tests/test_chain_analysis_backfill.py`: durable quota, scope/source fencing, revocation during transport, crash-conservative reservations, wrong genesis, repeated TXIDs, input occurrence resolution, Merkle mismatch, multi-step reorg and CLI/agent boundaries. Runtime lifecycle, protected-book and projection parity checks belong to the prerequisite watch/index suites and the combined integration checks.
