...
- A payjoin or coinjoin can be shown as a privacy-preserving hop, but the
  ...
  evidence. Wallet-specific privacy importers may add safe evidence that a
  transaction crossed a privacy boundary, but that evidence alone must not be
  expanded into exact participant lineage.
...
Coinjoin, payjoin, and wallet-specific privacy-hop nodes should collapse
