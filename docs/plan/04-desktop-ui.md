...
What this *doesn't* do: generate QML. HTML → QML is a manual translation step. Claude Design is a design-spec tool, not a codegen tool. Its output is the visual contract; QML is still written by hand (or by Claude Code consuming the screenshots). The Clams-fidelity tradeoffs from `01-stack-decision.md` (pastel-fill pills vs outlined pills, drop-shadow depth, serif weight) are best resolved in Claude Design before committing them to QML properties.
